...
const BookMarkedMeetingRecordList = () => {
    ...
    const { data = { meetingRecordList: [] } } = useQuery(['managementBookMarkMeetingRecordList', projectId, currentPage], () => getBookMarkMeetingRecordList(projectId, currentPage));

    useEffect(() => {
        const nextPage = currentPage + 1;

        if (nextPage <= data.totalPages) {
            queryClient.prefetchQuery(['managementBookMarkMeetingRecordList', projectId, nextPage], () => getBookMarkMeetingRecordList(projectId, nextPage));
        }
    ...
    return (
        <div className='meeting-record-book-marked-record-list'>
            ...
            <div className='meeting-record-body'>
                <MeetingRecordData
                    sortedMeetingRecordList={data.meetingRecordList}
                />
                <Pagination
