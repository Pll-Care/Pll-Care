...
public interface MidtermEvaluationRepository extends JpaRepository<MidtermEvaluation, Long> {
    ...
    @Query("select new fullcare.backend.evaluation.dto.MidtermBadgeListDao(me.voted.id, me.voted.name, me.evaluationBadge ,count(me.evaluationBadge)) from MidtermEvaluation me where me.project.id = :projectId and me.voted in :members group by me.evaluationBadge, me.voted.id order by me.voted.id")
    List<MidtermBadgeListDao> findList(Long projectId, List<Member> members);
}
